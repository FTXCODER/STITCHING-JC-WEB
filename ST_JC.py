...
# -------------------------------
start_index = 12  # Column M
step = 8

groups = []
i = start_index

while i + 2 < len(df.columns):
...
if "TIMESTAMP" in final_df.columns:
    final_df["TIMESTAMP"] = pd.to_datetime(final_df["TIMESTAMP"], format="%d/%m/%Y %H:%M:%S", errors="coerce")
...
st.write("Detected groups:", groups[:5])  # show first 5 only
